      ...
      <table className={AddClassTag(`MonthCalendar`)}> 
        <tbody>         
        {
              Util.ObjectKeys(this.state._allYear2Render).map((yr,inYr)=>{
                ...
                var dy2c = f_label?(new Date(f_label)):(new Date());
                    ...
                    <tr  key={`${inWk}@$week@00`} className={`${isWp?'weekActive':''}`} >
                    {
                      wk.map((dys,inDd)=>{                        
                        if(!weekPicker && dys.cls.indexOf(`dayNow`)>5){
                          return (
                            <td key={`${inDd}@$day@${dys.d}`} className={`${AddClassTag(dys.cls)} ${_month===parseInt(yr) && dys.d===day?'activeday':''}`} onClick={()=>{this.setDay(dys,yr)}}>
                          ...
                          if(dys.d){
                            return (
                              <td key={`${inDd}@$day@${dys.d}`} className={`${AddClassTag(dys.cls)} ${_month===parseInt(yr) && dys.d===day?'activeday':''}`} style={{width:`${wdt}px`}} onClick={()=>{this.setDay(dys,yr)}}>
                                <div  className={AddClassTag(`dayTextStyle`)}>
                                  {dys.d}
                                </div>
                              </td>
