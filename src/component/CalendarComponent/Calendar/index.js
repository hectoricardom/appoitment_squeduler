...
class Calendar extends Component {
  ...
  setDay= (r) => { 
    var _th6 = this;
    const { month,year} = _th6.props.calendar;   
    var cdate = `${month}/${r.d}/${year}`;
    this.props.actions.UpdateCalendarParams('day',r.d)
    if (typeof _th6.props.valueUpdate === 'function') {     
      _th6.props.valueUpdate(cdate,r);
    }  
  }
  ...
  render() {  
    ...
    var _colorRGBA = colorRGBA;
    if(Util.validateHex(theme)){
      var rgb = Util.hexToRgb(theme);
      _colorRGBA = `rgba(${rgb.r},${rgb.g},${rgb.b},0.35)`;
    }
    
    return (
      <div ref={this.ref} className="baseCalendar" style={{"--calendar--color--":currentColorTheme,'--calendar--back--color--':_colorRGBA}}> 
                      ...
                      <tr  key={`${inWk}@$week@00`}>
                          {
                            wk.map((dys,inDd)=>{
                              ...
                              if(dys.cls.indexOf(`dayNow`)>5){                                
                                return (
                                  <td key={`${inDd}@$day@${dys.d}`}  className={`${dys.cls} ${_month===month && dys.d===day?'activeday':''}`} onClick={()=>{this.setDay(dys)}}>
                                    <div  className='dayTextStyle'>
                                      {dys.d}
                                    </div>
                                  </td>
